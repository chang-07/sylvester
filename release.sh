...
if [ -d "/Library/Developer/SharedFrameworks/XCBuild.framework" ]; then
    SYLVESTER_BUILD_FLAGS="--arch arm64 --arch x86_64" ./make-app.sh
else
...
ARCHS=$(lipo -archs "$APP/Contents/MacOS/Sylvester" | tr ' ' '+')
DMG="dist/Sylvester-$VERSION-macos-$ARCHS.dmg"

echo "== architectures =="
lipo -archs "$APP/Contents/MacOS/Sylvester"
...
rm -f "$DMG" "$DMG.sha256"
hdiutil create -volname "Sylvester" -srcfolder "$STAGE" -ov -format UDZO "$DMG" >/dev/null
rm -rf "$STAGE"
